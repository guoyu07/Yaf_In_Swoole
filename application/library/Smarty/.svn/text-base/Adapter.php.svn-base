...
class Smarty_Adapter implements Yaf_View_Interface
{
    ...
    public function __construct($tmplPath = null, $extraParams = array()) {
        ...
        if (!file_exists($this->_smarty->cache_dir))
        {
            mkdir($this->_smarty->cache_dir);
        }
    }
    ...
    public function render($name = NULL, $value = NULL) {
        return $this->_smarty->fetch($name ? $name : $this->getScriptName());
    }

    public function display($name = NULL, $value = NULL) {
        echo $this->_smarty->fetch($name ? $name : $this->getScriptName());
    }

    /**
     * 根据当前的request找到对应的模板文件
     * @return [type] [description]
     */
    public function getScriptName(){
        ...
        return $this->getScriptPath().'/'.$request->controller.'/'.$request->action.'.'.$ext;
    }
}
?>